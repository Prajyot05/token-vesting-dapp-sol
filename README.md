...
Token vesting is a structured way to distribute tokens over time instead of giving the full allocation immediately.

In a typical project, a company may want to reward employees, advisors, or early investors with tokens while also ensuring long-term commitment. If all tokens are unlocked at once, recipients could sell immediately, which can create heavy selling pressure and destabilize token value.

Vesting solves this by introducing a release schedule. Stakeholders can only access a portion of their tokens at predefined times. This model:

- Aligns long-term incentives between contributors and the project
- Encourages sustained participation and commitment
- Reduces sudden token dumps in the market
- Improves trust in the token economy by making distribution predictable

In short, token vesting protects both the project and its community by balancing fair rewards with controlled, time-based token access.

## Getting Started

```shell
npx -y create-solana-dapp@latest -t solana-foundation/templates/kit/token-vesting-dapp
```
